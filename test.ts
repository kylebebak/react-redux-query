...
test('save reducer', async (t) => {
  const newState = reduce({}, { type: 'REACT_REDUX_QUERY_SAVE', payload: { key: 'res', response: { data: true } } })
...
test('save action creator', async (t) => {
  ...
  t.deepEqual(action, { type: 'REACT_REDUX_QUERY_SAVE', payload: { key: 'res', response: {} } })
})

test('update reducer', async (t) => {
  const state = { res: { data: true } }

  let newState = reduce(state, { type: 'REACT_REDUX_QUERY_UPDATE', payload: { key: 'res', updater: () => undefined } })
  t.is(newState.res?.data, true)
